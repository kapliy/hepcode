...
for iname in allnames:
    ...
    if re.match('mc',iname):
        hmcP[iname]=ScaleToLumi(pos.Get(hname).Clone(),iname,opts.lumi,opts.qcdscale,nevts)
        hmcP[iname].Sumw2()
        hmcM[iname]=ScaleToLumi(neg.Get(hname).Clone(),iname,opts.lumi,opts.qcdscale,nevts)
        hmcM[iname].Sumw2()
        if re.search('wmunu',iname):
            ...
            pos = g.GetDirectory('POS')
            neg = g.GetDirectory('NEG')
            htrP = ScaleToLumi(pos.Get(hname).Clone(),iname,opts.lumi,opts.qcdscale,nevts)
            htrP.Sumw2()
            htrM = ScaleToLumi(neg.Get(hname).Clone(),iname,opts.lumi,opts.qcdscale,nevts)
            htrM.Sumw2()
    else: #data:
