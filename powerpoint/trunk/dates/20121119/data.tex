...
{
\centering
\large New $\eta-\phi$ muon trigger map
}
...
\only<4>{\footnotesize{But adding the new $\phi$ corrections does not seem to make it better. Bug? }}
}
...
\only<1>{\footnotesize{Note that trigger scale factors reduce MC cross-section by 1.4\%.}}
...
{
\centering
\large Which nevents to use to normalize Monte-Carlo to luminosity?
}
...
{
\centering
\large Checking the effect of various reweighting tools
}
...
{
\centering
\large Lepton $p_{T}$ plots with different variations
}
