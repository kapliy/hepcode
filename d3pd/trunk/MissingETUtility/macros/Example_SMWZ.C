...
void Example::Loop()
{
  ...
  JERProvider _JER("AntiKt6LCTopoJES", "Truth", "../../Jet/JetResolution/share/JERProviderPlots.root");
  _JER.init();
  ...
  //start of loop/ntuple stuff
   ...
   for (Long64_t jentry=0; jentry<1000;jentry++) {
	...
	float jerShift = 1.0;//this is just a default value for out of bound jets. Chosen from the value for eta > 2.1 to be conservative.
	...
	if(jet_AntiKt4TopoNewEM_pt->at(iJet) > 15000.0
	  ...
	  if( jet_AntiKt4TopoNewEM_pt->at(iJet)>20000) {
	    for ( unsigned int ii = 0; ii < jet_AntiKt4TopoNewEM_n->size(); ii++ ) {
	      if(jet_AntiKt4TopoNewEM_emscale_pt->at(ii)>7000) {
		if(i!=ii) {
		  double dr = deltaR(jet_AntiKt4TopoNewEM_eta->at(iJet), 
				     jet_AntiKt4TopoNewEM_phi->at(iJet), 
				     jet_AntiKt4TopoNewEM_eta->at(ii),
				     jet_AntiKt4TopoNewEM_phi->at(ii));
...
//       metCheck->setMETTerm("CellOut", MET_CellOut_etx, MET_CellOut_ety, MET_CellOut_sumet);
	...
	cout << "+++++++++++++++++++++++++++++" << endl;
	 ...
	 cout << "+++++++++++++++++++++++++++++" << endl;
      ...
      metSys->setObjects("jets", jet_AntiKt4LCTopo_pt, jet_AntiKt6LCTopo_eta, jet_AntiKt6LCTopo_phi, jet_AntiKt6LCTopo_E, jet_AntiKt6LCTopo_MET_wet, jet_AntiKt6LCTopo_MET_wpx, jet_AntiKt6LCTopo_MET_wpy, jet_AntiKt6LCTopo_MET_statusWord);
