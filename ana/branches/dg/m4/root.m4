...
[
  ...
  if test ! x"$ROOTCONF" = "xno" && \
    ...
    rootlibstmp=`$ROOTCONF --noauxlibs --noldflags --libs`
    rootglibstmp=`$ROOTCONF --noauxlibs --noldflags --glibs`
    ROOTLIBS="$rootlibstmp -lEG"
    ROOTGLIBS="$rootglibstmp -lEG"
    ...
    ROOTNUMVERSION=`$ROOTCONF --version | tr './' ' ' | awk 'BEGIN { FS = " "; } { printf "%d", ($''1 * 1000 + $''2) * 1000 + $''3;}'`
  ...
  #AC_CONDITIONAL(ROOFIT_VERSION3,[test $ROOTNUMVERSION -ge 5024000])
  if test $ROOTNUMVERSION -ge 5024000 ; then
     ROOFITLIBS="-lRooFit -lRooFitCore -lMinuit -lFoam"
     AC_MSG_NOTICE(Using RooFit version 3 library scheme)
  else 
     ROOFITLIBS="-lRooFit"
     AC_MSG_NOTICE(Using old RooFit library scheme)
  fi
