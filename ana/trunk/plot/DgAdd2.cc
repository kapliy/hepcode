...
map<string,bool> allNames; // all TObjects
map<string,vector<string> > dirMap;   // a map of objects inside each TDirectory
unsigned int ITER = 0;
std::string TMPROOT = "";
...
// recursive ls function that transverses the input files and comes up
// with a union-type listing of all TObjects
int MakeDirMap( TDirectory *targetDG, TDirectory *targetROOT, TList *sourcelist) {
  //  cout << "Target path: " << targetDG->GetPath() << endl;
  ...
  while ( (key = (TKey*)nextkey())) {
	...
	newdirDG = targetDG->mkdir( name, obj->GetTitle() );
      ...
      if(targetROOT) {
	targetROOT->cd();
	if (allNames.find(fullpath)==allNames.end()) newdirROOT = targetROOT->mkdir( name, obj->GetTitle() );
	else newdirROOT = targetROOT->GetDirectory(name);
      }
      allNames[fullpath]=true;
