...
class
StudyTrackingEfficiency : public DataGraphics::DgStudy< std::pair< boost::shared_ptr<const AnaTrack> , bool > >
{
public:
...
protected:
  virtual void _study( type const& value ) { // the second const is important---matches the base class.
    ...
    if( _event && _event->highest_sum_pt_vertex() ) { 
      ...
      dg::filleff( "eff_vs_d0raw" , 400 , -1 , 1 , 
		   reference_track->d0_raw( _event->highest_sum_pt_vertex()->x() , 
					    _event->highest_sum_pt_vertex()->y() )/10. , 
		   success , reference_track->algorithm().name() + " d_{0} (cm)" );
      dg::filleff( "eff_vs_z0raw" , 100 , -15 , 15 , 
		   reference_track->z0_raw( _event->highest_sum_pt_vertex()->z() )/10. , 
		   success , reference_track->algorithm().name() + " z_{0} (cm)" );
    }
//     if( _event ) {
...
private:
  ...
  bool _is_fake_rate;
public:
  StudyTrackingEfficiency( const std::string& name , const std::string& description ) 
    : DataGraphics::DgStudy<type>(name,description)
    , _event()
    , _is_fake_rate( false )
  {}
  virtual ~StudyTrackingEfficiency() {}
