...
#include "TrigFTKAna/Trees/D3PDJetMETTreeBase.h"
...
NtD3PDphysJetMET::NtD3PDphysJetMET()
  : NtD3PDphys(NtD3PDphys::JETMET),
    _open_filename(),
    _n_events(0ul),
    _run_number(0ul),
    _event_number(0ul),
...
const bool NtD3PDphysJetMET::begin_file( const std::string& raw_filename )
{ 
  ...
  if( !_file ) 
    { 
      cout << " NtD3PDphysJetMET: unable to open file " << filename << endl;
      return false;
    }
  assert( _file );
  TTree* tree = dynamic_cast<TTree*>(_file->Get("qcd"));
  if( !tree )
    { 
      cout << " NtD3PDphysJetMET: unable to find physics tree file " << filename << endl;
      return false;
    }
  
  // Version 1 corresponds to MC12 D3DPs from the Jet/Etmiss requests:
  // https://twiki.cern.ch/twiki/bin/viewauth/AtlasProtected/JetEtmissMC12
  _madeclass_version = V1;
  AnaConfiguration::release(17);
  cout << " NtD3PDphysJetMET: configured for V1 ntuple." << endl;
  
  switch( _madeclass_version ) 
    { 
    case V1: _madeclass.reset( new D3PDJetMETTreeV1(tree) ); break;
  ...
  if( !tree->GetCurrentFile() || tree->GetCurrentFile()->IsZombie() || !(tree->GetCurrentFile()->IsOpen()) ) 
    { 
      cout << " could not open JetMET D3PD: " << ana_streams::root_dcap_filename(filename) << endl;
      return false;
    }
  if( _n_events == 0 ) 
    {
      cout << " empty JetMET D3PD: " << filename << endl;
      return false;
    }
...
const bool NtD3PDphysJetMET::end_file()
{
  _n_events = 0;
...
void NtD3PDphysJetMET::set_enabled_branches()
{ }

void NtD3PDphysJetMET::set_branch_addresses()
{ }

const bool NtD3PDphysJetMET::get_event( const unsigned long& ievent )
{
  ...
  switch( _madeclass_version ) 
    {
    case V1: return _get_event_V1(); break;
    default: assert( !"unhandled case" );
    }
...
const unsigned long NtD3PDphysJetMET::count_events(const std::string& filename)
{
  ...
  TTree* tree = dynamic_cast<TTree*>(f->Get("qcd"));
  if( !tree ) { 
    cout << " NtD3PDphysJetMET: unable to find \"qcd\" tree in file " << filename << endl;
    return 0;
  }
  const unsigned long n = tree->GetEntries();
  delete f;
  cout << " NtD3PDphysJetMET: file " << filename << " contains " << n << " events." << endl;
  return n;
}

const bool NtD3PDphysJetMET::_get_event_V1() 
{ 
  const D3PDJetMETTreeV1* nt( dynamic_cast<const D3PDJetMETTreeV1*>(_madeclass.get()) );
  assert( nt );
  ...
  // identify highest-su-mpt primary vertex for lepton track exPV d0/z0                                                                                                                                   
  ...
  // Prepare a map of barcode -> index
  std::map<int,int> mc_barcode2idx;
  const vector<int>& mc_barcodes=(*nt->mc_barcode);
  for(unsigned int mcidx=0;mcidx<nt->mc_n;mcidx++)
    {
      mc_barcode2idx[mc_barcodes[mcidx]]=mcidx;
    }
  ...
  for( int imc=0, fmc=nt->mc_n; imc!=fmc; ++imc ) {
    ...
    const vector<int>& parents( (*(nt->mc_parents))[imc] );
    const int jmc = parents.empty() ? -1 : mc_barcode2idx[parents.front()]; // take first parent in list. see truth table for comprehensive parentage.
  ...
  if( true ) { // FIXME this section causes crashes when running over some backgrounds, can switch to false temporarily
    ...
    for( AnaTruthTable::index_type imc=0, fmc=nt->mc_n; imc!=fmc; ++imc ) {
      ...
      const vector<int>& parents( (*(nt->mc_parents))[imc] );
      BOOST_FOREACH(int parent, parents)
	{
	  mos.push_back( parent );
	}

      const vector<int>& children( (*(nt->mc_children))[imc] );
    ...
    if( false ) { // for debugging
      std::cout << " begin truth table for NtD3PDphysJetMET " << endl;
      _truth_table->print_cdf_style( std::cout );
      ...
      std::cout << " end truth table for NtD3PDphysJetMET " << endl;
    }
...
void NtD3PDphysJetMET::_make_associated_track_for_electron(shared_ptr<AnaElectron>& ele, 
...
void  NtD3PDphysJetMET::_make_associated_track_for_muid_muon(shared_ptr<AnaMuon>& mu, 
							     const D3PDJetMETTreeV1 *nt,
							     const unsigned int& i, 
							     const shared_ptr<const AnaVertex>& vx )
{
...
void NtD3PDphysJetMET::_make_associated_track_for_staco_muon(shared_ptr<AnaMuon>& mu, 
							     const D3PDJetMETTreeV1* nt,
							     const unsigned int& i, 
							     const shared_ptr<const AnaVertex>& vx)
{
...
void NtD3PDphysJetMET::_make_associated_track_for_calo_muon(shared_ptr<AnaMuon>& mu, 
							    const D3PDJetMETTreeV1* nt,
							    const unsigned int& i, 
							    const shared_ptr<const AnaVertex>& vx)
{
...
void NtD3PDphysJetMET::_make_vertices(const D3PDJetMETTreeV1* nt)
{
  assert( nt );
