...
public:
  ...
  virtual RooUnfoldResponse& Setup (const TH1* measured, const TH1* truth, const TH2* response, const TH1* fakes);  // set up from already-filled histograms
  virtual RooUnfoldResponse& Setup (const TH1* measured, const TH1* truth, const TH2* response) { // legacy compatibility
    return Setup(measured,truth,response,0);
  }
  ...
  virtual void Add (const RooUnfoldResponse& rhs , Double_t scale = 1.0 );
