...
void
RooUnfoldResponse::Add (const RooUnfoldResponse& rhs, Double_t scale)
{
  ...
  _mes->Add (rhs._mes , scale);
  _fak->Add (rhs._fak , scale);
  _tru->Add (rhs._tru , scale);
  _res->Add (rhs._res , scale);
}
...
RooUnfoldResponse::Setup (const RooUnfoldResponse& rhs)
{
  ...
  return Setup (rhs.Hmeasured(), rhs.Htruth(), rhs.Hresponse() , rhs.Hfakes());
}
...
RooUnfoldResponse&
RooUnfoldResponse::Setup (const TH1* measured, const TH1* truth, const TH2* response, const TH1* fakes)
{
  ...
  if ( !measured || (_mes->GetEntries() == 0.0 && _res->GetEntries()>0) ) {
  ...
  if ( !truth || (_tru->GetEntries() == 0.0 && _res->GetEntries()>0) ) {
