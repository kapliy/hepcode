...
bool RUNZ = true;                      // run Z selection and histograms?
bool RUNZ_NT = true;                   // save Z ntuples? This is set automatically for MCP studies
...
#undef JET
  ...
  for( AnaEventMgr::iterator i=evtmgr->begin(), f=evtmgr->end(AnaConfiguration::user_max_nevents()); i!=f; ++i, ++n_events ) {
	...
	if(mc_type == MC_POWHEG_PYTHIA && PYTHIA_TO_HERWIG) {
...
#undef UNF_TR
  ...
  // Z ntuples
  dg::set_global_weight( event_weight[0]*eff*trig*siso );
  if(RUNZ && e_met_cleaning && e_lar_hole && mu_ntuple_col.size()>=2 ) {
    MAKE_Z(ntuple);
    // study z's
