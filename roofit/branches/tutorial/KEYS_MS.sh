...
for np in $wfiles; do
    ...
    ./keysfit.py --root root_all_v1.root --batch --tag ms_${evtype}_${reg}_${nevt} --data ${subfile}/dg/dg/st_z_final/$reg/graph_lptms_P_N --ndata $nevt --rookeys wspace/ms_${reg}_${evtype}${nevt}.root --nscan 100 --scan ${xtra}
    if [ "$evtype" == "data" -o "$evtype" == "mc" ]; then
	./keysfit.py --root root_all_v1.root --batch --tag ms_${evtype}_${reg}_${nevt}_ks --data ${subfile}/dg/dg/st_z_final/$reg/graph_lptms_P_N --ndata $nevt --nscan 100 --scan ${xtra} --kolmogorov
    fi;
done
