...
void rf109_chi2resid_modified()
{
  ...
  TCanvas* c = new TCanvas("rf109_chi2residpull","rf109_chi2residpull",300,600) ;
  TPad*    p1 = new TPad("p1", "", .005, .2525, .995, .995);
  TPad*    p2 = new TPad("p2", "", .005, .005, .995, .2475);
  p1->Draw();        
  p2->Draw(); 

  p1->cd() ; gPad->SetLeftMargin(0.15) ; frame1->GetYaxis()->SetTitleOffset(1.6) ; frame1->Draw() ;
  p2->cd() ; gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.6) ; frame2->Draw() ;
