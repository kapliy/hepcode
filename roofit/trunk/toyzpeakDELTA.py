...
# THIS VERSION APPLIES IT TO A DELTA FUNCTION
...
parser.add_option("--root",dest="root",
                  type="string", default='ROOT/root_all_0522_dataMCclosure.root',
...
parser.add_option("--ievent",dest="ievent",
                  type="int", default=1,
                  help="Which event to take as a baseline for toy MC")
...
if True:
    ...
    N,Zs = ntuple_to_4vectors(hz,opts.region,opts.min,opts.max,maxdata=opts.ievent+10)
    ...
    M0 = (POS+NEG).M()
    print 'Base event has pt = %.1f and %.1f ; z mass = %.2f'%(POS.Pt(),NEG.Pt(),M0)
    ptPOS,ptNEG = Zs[i][0][0],Zs[i][1][0]
    etaPOS,etaNEG = Zs[i][0][1],Zs[i][1][1]
    c = ROOT.TCanvas('c','c')
    c.cd()
    h = ROOT.TH1F('h','h',100,M0-0.5,M0+0.5)
    for i in range(0,opts.ndata):
    ...
    #h.Fit("gaus")
    line = ROOT.TGraph(2)
    line.SetPoint(0,M0,0)
    line.SetPoint(1,M0,h.GetMaximum())
    line.SetLineWidth(4)
